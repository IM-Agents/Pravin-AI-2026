...
# Local Postman/curl only: skip HMAC check when NODE_ENV is not production
...
| `SHOPIFY_SKIP_WEBHOOK_VERIFY` | Set `true` to skip HMAC in dev (ignored if `NODE_ENV=production`) | No | false |
...
### Shopify → your machine (production-like)

Use a tunnel (for example ngrok) and register the URL in Shopify:

```bash
ngrok http 8000
```

Point Shopify webhooks at `https://<your-tunnel>/api/webhooks/shopify/orders/create` (and `/orders/cancelled` as needed). Shopify sends a valid `X-Shopify-Hmac-Sha256` header automatically.

### Manual order create (`curl` / Postman)

**Endpoint:** `POST http://localhost:8000/api/webhooks/shopify/orders/create`  
**Headers:** `Content-Type: application/json`  
**Body:** Shopify-shaped JSON (minimal example below).

**HMAC and headers**

- If `SHOPIFY_WEBHOOK_SECRET` is **unset**, the server skips signature validation but you must still send a non-empty `X-Shopify-Hmac-Sha256` header (any value, e.g. `dev`).
- If `SHOPIFY_WEBHOOK_SECRET` **is** set, the header must be **Base64(HMAC-SHA256(raw body bytes, secret))** matching the exact JSON you send. For local testing without computing HMAC, set `SHOPIFY_SKIP_WEBHOOK_VERIFY=true` in `.env` (only works when `NODE_ENV` is not `production`).

**Payload notes**

- Use a **new numeric `id`** for each test; repeating an existing `order_id` returns `200` with “Order already exists”.
- `created_at` may be ISO-8601 (for example `2024-01-15T10:30:00Z`); the backend stores it as MySQL `DATETIME`.

Save as `order-payload.json`:
  ...
  "line_items": [
    {
      ...
      "variant_id": 333333,
      "properties": [
        { "name": "Delivery Date", "value": "20/01/2024" },
        { "name": "Delivery Time", "value": "Morning" }
      ]
    }
  ],
  "note_attributes": [{ "name": "Delivery Date", "value": "20/01/2024" }]
}
```
...
```bash
curl -sS -X POST "http://localhost:8000/api/webhooks/shopify/orders/create" \
  -H "Content-Type: application/json" \
  -H "X-Shopify-Hmac-Sha256: dev" \
...
```powershell
curl.exe -sS -X POST "http://localhost:8000/api/webhooks/shopify/orders/create" `
  -H "Content-Type: application/json" `
  -H "X-Shopify-Hmac-Sha256: dev" `
  -d "@order-payload.json"
```

**Valid HMAC when `SHOPIFY_WEBHOOK_SECRET` is set** (must hash the **exact** bytes you POST). With `order-payload.json` in the **repository root** (next to `backend/`):

```bash
cd backend
node -e "require('dotenv').config();const c=require('crypto'),fs=require('fs'),p=require('path');const b=fs.readFileSync(p.resolve('..','order-payload.json'));process.stdout.write(c.createHmac('sha256',process.env.SHOPIFY_WEBHOOK_SECRET).update(b).digest('base64'))"
```

Use the printed string as `X-Shopify-Hmac-Sha256` and POST that same file as the body (no extra spaces or reformatting).
